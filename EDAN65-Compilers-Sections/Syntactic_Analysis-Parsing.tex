...
\begin{definition}[FIRST]\label{def:FIRST}
  ...
  The $\text{FIRST}(\gamma)$ are the tokens that occur \emph{\nameref{def:FIRST}} in a sentence derived from $\gamma$.
  ...
  \begin{subequations}
    \begin{equation}\label{eq:FOLLOW_Epsilon}
      \FIRST(\epsilon) = \emptyset
    \end{equation}
    \begin{equation}\label{eq:FOLLOW_Terminal}
      \FIRST(t) = \lbrace t \rbrace
    \end{equation}
    where $t \in T$, i.e., $t$ is a terminal symbol

    \begin{equation}\label{eq:FOLLOW_Nonterminal}
      \FIRST(X) = \FIRST(\gamma_{1}) \cup \ldots \cup \FIRST(\gamma_{n})
    \end{equation}
    where $X \rightarrow \gamma_{1}, \ldots, X \rightarrow \gamma_{n}$ are all the productions for $X$ in $P$.

    \begin{equation}\label{eq:FOLLOW_Nonterminal_or_Terminal}
  ...
  \begin{remark}
    The equations (\Crefrange{eq:FOLLOW_Epsilon}{eq:FOLLOW_Nonterminal_or_Terminal}) for \nameref{def:FIRST} are recursive.
...
\begin{blackbox}
  ...
  \nameref{def:FOLLOW} is shown below. I will use this grammar to
  demonstrate:
  \begin{align*}
    S &\rightarrow Xa \\
    X &\rightarrow Y \vert Yb \\
    Y &\rightarrow YZc \vert \epsilon \\
    Z &\rightarrow d \vert \epsilon
  \end{align*}
  \begin{enumerate}[noitemsep]
  \item Determine the nonterminal you are interested in, let's say
    $Y$.
  \item Find all occurrences of that nonterminal on the
    \textbf{\emph{RIGHT-HAND SIDE}} of the productions. In this case
    our nonterminal occurs in:
    \begin{itemize}[noitemsep]
    \item Production 2
    \item Production 3
    \end{itemize}
  \item Find the nonterminals that can directly follow your
    nonterminal.
    \begin{itemize}[noitemsep]
    \item $b$, from Production 2
    \item $c$, from Production 3
    \end{itemize}
  \item If there are nonterminals after the nonterminal you are
    interested in, follow them. In this case, we follow $Z$. Then,
    find the nonterminals that can directly follow that nonterminal
    and add them to you're list.
    \begin{itemize}[noitemsep]
    \item $b$, from Production 2
    \item $c$, from Production 3
    \item $d$, from Production 4
    \end{itemize}
  \item If no token follows the nonterminal you want, then go
    ``backwards'' through the production.
    \begin{enumerate}[noitemsep]
    \item Since Production 2 has $X \rightarrow Y$ as an option and
      nothing follows this $Y$
    \item Go backwards, up to the production(s) that produces $X$ on
      the right-hand side
    \item Compute Follow on the right-hand side of that production.
      This produces:
      \begin{itemize}[noitemsep]
      \item $a$, from Production 1
      \end{itemize}
    \end{enumerate}
    This leaves us with our $\FOLLOW(Y)$ set:
    $\lbrace a, b, c, d \rbrace$, which is the solution.
  \end{enumerate}
\end{blackbox}
