...
\begin{definition}[Context-Free Grammar]\label{def:Context_Free_Grammar}
  ...
  \begin{remark}
    It is important to note that there are 3 forms of \nameref{def:Context_Free_Grammar}s:
    \begin{enumerate}[noitemsep]
...
It is called the \emph{\nameref{subsubsec:Formal_Grammar_Hierarchy}}.
\begin{table}[h!]
  \centering
  \begin{tabular}{ccc}
    \toprule
...
However, the $\LLParse(1)$ table \textbf{\emph{DOES NOT}} show that a grammar is ambiguous; it only shows that that \nameref{def:Context_Free_Grammar} is not $\LLParse(1)$.

\subsubsection{Issues with \nameref{subsec:LLParsing}}\label{subsubsec:LLParsing_Issues}
There are 2 major issues with \nameref{subsec:LLParsing}.
\begin{enumerate}[noitemsep]
\item \nameref{par:LLParsing_Issues_Common_Prefix}
\item \nameref{par:LLParsing_Issues_Left_Recursion}
\end{enumerate}

\paragraph{Common Prefix}\label{par:LLParsing_Issues_Common_Prefix}
\begin{definition}[Common Prefix]\label{def:Common_Prefix}
  A \emph{common prefix} issue is one in which multiple productions that start from the same \nameref{def:Nonterminal_Symbol} \emph{start} with the same \nameref{def:Nonterminal_Symbol} or \nameref{def:Terminal_Symbol}.
\end{definition}

The below grammar is an example of a \emph{direct} \nameref{def:Common_Prefix} issue:
...
The below grammar is an example of an \emph{indirect} \nameref{def:Common_Prefix} issue:
...
\begin{definition}[Left Recursion]\label{def:Left_Recursion}
  A \emph{left recursion} issue is one where a set of productions can eventually yield the same \nameref{def:Nonterminal_Symbol} as was started with.

  \begin{remark}
    Every \nameref{par:LLParsing_Issues_Left_Recursion} is a special case of \nameref{par:LLParsing_Issues_Common_Prefix}s where the \nameref{def:Nonterminal_Symbol}s are the common element.
  \end{remark}
\end{definition}

The below grammar is an example of a \emph{direct} \nameref{def:Left_Recursion} issue:
...
\begin{equation}\label{eq:Indirect_Left_Recursion}
  ...
  \end{aligned}
\end{equation}
In \Cref{eq:Indirect_Left_Recursion}, if you perform a left-most derivation, you will recurse down $X$ permanently.

\subsubsection{Eliminating \nameref{subsubsec:LLParsing_Issues}}\label{subsubsec:Eliminate_LLParsing_Issues}
The goal when eliminating $\LLParse(1)$ parser issues is to generate an \nameref{def:Equivalent_Grammar}, while fixing the issues present in the original grammar.

\begin{definition}[Equivalent \nameref{def:Context_Free_Grammar}]\label{def:Equivalent_Grammar}
  Two \nameref{def:Context_Free_Grammar}s are said to be  \emph{equivalent} if they both produce the same \nameref{def:Language}.
  Technically, this is another unsolvable problem for \nameref{def:Context_Free_Grammar}s that generate an infinitely large \nameref{def:Language}s.
  However, you can show that they \emph{may} be equivalent by using example cases.
\end{definition}

\paragraph{Eliminate \nameref{par:LLParsing_Issues_Common_Prefix}}\label{par:Eliminate_Common_Prefix}
Since \nameref{def:Common_Prefix} issues rely in two different productions starting with the same \nameref{def:Terminal_Symbol}, if you combine the productions into one, and create a new production to handle the unique cases, then you can solve the issue.

\begin{example}[Exercise 13, Problem 4]{Eliminate Common Prefix}
  The following grammar has a common prefix problem.
  Transform the grammar to an \nameref{def:Equivalent_Grammar} where the common prefix is eliminated.

  \begin{equation*}
    \begin{aligned}
      p_{0} &: G \rightarrow \text{ElementList} \\
      p_{1} &: \text{ElementList} \rightarrow \text{Element } \text{ElementList} \\
      p_{2} &: \text{ElementList} \rightarrow \epsilon \\
      p_{3} &: \text{Element} \rightarrow \text{Node} \\
      p_{4} &: \text{Element} \rightarrow \text{Edge} \\
      p_{5} &: \text{Node} \rightarrow \text{ID} \\
      p_{6} &: \text{Edge} \rightarrow \text{ID ``('' ID ``$\rightarrow$'' ID ``)''} \\
    \end{aligned}
  \end{equation*}

  \tcblower{}

  The \nameref{def:Common_Prefix} in this grammar is an indirect one.
  If you follow $p_{3}$ and $p_{4}$, then $p_{5}$ and $p_{6}$ are the issue.

  We start by removing the redundant productions to make things a bit clearer.
  \begin{equation*}
    \begin{aligned}
      p_{0} &: G \rightarrow \text{ElementList} \\
      p_{1} &: \text{ElementList} \rightarrow \text{Element } \text{ElementList} \\
      p_{2} &: \text{ElementList} \rightarrow \epsilon \\
      p_{3} &: \text{Element} \rightarrow \text{ID} \\
      p_{4} &: \text{Element} \rightarrow \text{ID ``('' ID ``$\rightarrow$'' ID ``)''} \\
    \end{aligned}
  \end{equation*}

  The \nameref{def:Common_Prefix} issue is more obvious now.
  Like said earlier, we can remove the issue by factoring the unique terms out to their own productions and leaving the common elements alone.
  In this case, that means we combine $p_{3}$ and $p_{4}$ and leave the ID alone.
  However, we factor out the unique elements in $p_{3}$ and $p_{4}$ with their own productions.
  \begin{equation*}
    \begin{aligned}
      p_{0} &: G \rightarrow \text{ElementList} \\
      p_{1} &: \text{ElementList} \rightarrow \text{Element } \text{ElementList} \\
      p_{2} &: \text{ElementList} \rightarrow \epsilon \\
      p_{3} &: \text{Element} \rightarrow \text{ID } \text{ElementRest} \\
      p_{4} &: \text{ElementRest} \rightarrow \epsilon \\
      p_{5} &: \text{ElementRest} \rightarrow \text{``('' ID ``$\rightarrow$'' ID ``)''} \\
    \end{aligned}
  \end{equation*}
\end{example}

\paragraph{Eliminate \nameref{par:LLParsing_Issues_Left_Recursion}}\label{par:Eliminate_Left_Recursion}
$\LLParse(1)$ parsers cannot support \nameref{def:Left_Recursion}, however, they can support right recursion.
...
\begin{example}[Exercise 13, Problem 5]{Eliminate Left Recursion}
  The following grammar is left-recursive.
  Transform the grammar into an \nameref{def:Equivalent_Grammar}.

  \begin{equation*}
    \begin{aligned}
      p_{0} &: T \rightarrow T \text{ ``*'' } F \\
      p_{1} &: T \rightarrow F \\
      p_{2} &: T \rightarrow \text{ID} \\
      p_{3} &: T \rightarrow \text{``('' } T \text{ ``)''} \\
    \end{aligned}
  \end{equation*}
  
  \tcblower{}

  This is a simple case where we want to \nameref{par:Eliminate_Left_Recursion}.
  Since this is for a $\LLParse(1)$ parser, we can simply flip $p_{0}$ to make the new grammar right-recursive.
  \begin{equation*}
    \begin{aligned}
      p_{0} &: T \rightarrow F \text{ ``*'' } T \\
      p_{1} &: T \rightarrow F \\
      p_{2} &: T \rightarrow \text{ID} \\
      p_{3} &: T \rightarrow \text{``('' } T \text{ ``)''} \\
    \end{aligned}
  ...
  This new grammar has a \nameref{def:Common_Prefix} issue, but we leave that for the reader to solve.
  \textit{Note: It is identical to \Cref{ex:Eliminate Common Prefix}}.
\end{example}
...
\begin{example}[]{LR1 Shift-Reduce Parsing}
  Say you have a set of productions as follows:
  \begin{align*}
    p_{1}&: X \rightarrow YZV \\
    p_{2}&: Y \rightarrow ab \\
    p_{2}&: Z \rightarrow c \\
    p_{3}&: V \rightarrow de
  \end{align*}
  ...
  \item Construct your data structures.
    \begin{equation*}
      \underset{Stack\:}{\underbrace{\bullet}} \underset{Queue}{\underbrace{abcde}}
    \end{equation*}
...
\begin{definition}[LR Item]\label{def:LRItem}
  ...
  \begin{itemize}[noitemsep]
  \item A \emph{dot}  ($\bullet$), corresponding to the position in the input sentence (and the token at the top of the stack).
...
The resulting parser is called a \emph{Lookahead $\LRParse(1)$}, \emph{$\text{LALR}(1)$}, parser.
...
There are some things that \nameref{def:Context_Free_Grammar}s cannot describe, and thus \textbf{\emph{CAN}} parsed correctly, but make no sense \textbf{\emph{SEMANTICALLY}}.
