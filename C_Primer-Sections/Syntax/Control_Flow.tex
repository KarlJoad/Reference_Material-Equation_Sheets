...
\paragraph{\texorpdfstring{\cinline{do while}}{\texttt{do while}}}\label{par:dowhile_Loop}
The \cinline{do while} loop performs some action an uncountable or unknown number of times, \textbf{AT LEAST} once.
This is typically used when the action to be repeated can be repeated any number of times, but must happen at least once.
The syntax for this is shown in \Cref{lst:dowhile_Loop}.

\begin{listing}[h!tbp]
\csourcefile{./C_Primer-Sections/Syntax/Code/do-while-loop.c}
\caption{\texorpdfstring{\cinline{do while}}{\texttt{do while}} Loop Syntax}
