...
\begin{definition}[Perfect Secrecy]\label{def:Perfect_Secrecy}
  ...
  Alternatively, \Cref{eq:Perfect_Secrecy_2} is true.
  \begin{equation}\label{eq:Perfect_Secrecy}
    \MutualInformation(M;C) = 0 = H(M) - H(M \Given C)
  \end{equation}
  \begin{equation}\label{eq:Perfect_Secrecy_2}
    \Entropy(K) \geq \Entropy(M_{1}, M_{2}, \ldots, M_{n})
  \end{equation}
\end{definition}
...
\begin{definition}[\nameref*{def:Entropy} Per Alphabet Symbol]\label{def:Entropy_per_Alphabet_Symbol}
  ...
  \begin{remark}[Cases of \nameref*{def:Entropy_per_Alphabet_Symbol}]\label{rmk:Entropy_per_Alphabet_Symbol-Cases}
    There are 2 cases:
    \begin{enumerate}[noitemsep]
    \item A memoryless source, meaning each message is sent independently or each other.
      \begin{equation}\label{eq:Entropy_per_Alphabet_Symbol-Memoryless}
        \Entropy(M) = N \Entropy(\mathbf{M})
      \end{equation}
...
\begin{theorem}[Drawback of \nameref*{def:Perfect_Secrecy}]\label{thm:Perfect_Secrecy_Drawback}
  For a cryptosystem with \nameref{def:Perfect_Secrecy}, we have
  \begin{equation*}
    \Entropy(\mathbf{M}) \leq \Entropy(K)
  \end{equation*}
\end{theorem}

\begin{proof}[Drawback of \nameref*{def:Perfect_Secrecy}]\label{proof:Perfect_Secrecy_Drawback}
  We know
  \begin{equation*}
    \Entropy(\mathbf{M} \Given \mathbf{C}) \leq \Entropy(K \Given \mathbf{C}) \leq \Entropy(K)
  \end{equation*}

  But the definition of \nameref{def:Perfect_Secrecy} (\Cref{def:Perfect_Secrecy}) implies
  \begin{equation*}
    \Entropy(\mathbf{M} \Given \mathbf{C}) = \Entropy(\mathbf{M})
  \end{equation*}
  So, \Cref{thm:Perfect_Secrecy_Drawback} follows.
  \begin{equation*}
    \Entropy(\mathbf{M}) \leq \Entropy(K \Given \mathbf{C}) \leq \Entropy(K)
  \end{equation*}
\end{proof}
%%% Local Variables:
%%% mode: latex
