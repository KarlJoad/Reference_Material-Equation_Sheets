...
\begin{definition}[Hash Function]\label{def:Hash_Function}
  ...
  \item \nameref{def:Collision}-resistant
  \end{enumerate}
\end{definition}

\begin{definition}[Preimage Resistant]\label{def:Preimage_Resistant}
  A \nameref{def:Hash_Function} with an output hash value of $n$ bits, is considered \emph{preimage resistant} if the time complexity for finding the input is $O(2^{n})$.
\end{definition}
