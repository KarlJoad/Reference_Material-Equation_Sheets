...
Before getting to the discussion of cellular generations, we need to mention \nameref{def:Heterogenous_Network}s.

\begin{definition}[Heterogenous Networks]\label{def:Heterogenous_Network}
...
\item The \nameref{def:Mobile_Station}
\item The \nameref{par:Base_Station_Subsystem}
\item The \nameref{par:Network_Station_Subsystem}
\item The \nameref{par:Operation_Station_Subsystem}
\end{enumerate}

\begin{definition}[Mobile Station]\label{def:Mobile_Station}
  ...
  Mobile stations typically have a \nameref{def:SIM} installed to identify users.
\end{definition}

\begin{definition}[Subscriber Identification Module]\label{def:SIM}
...
\begin{definition}[Base Station Subsystem]\label{def:Base_Station_Subsystem}
  The \emph{Base Station Subsystem} (\emph{BSS}) is the main radio interface for the \nameref{def:Mobile_Station}s.
  It takes in the cellular signals from these devices and connects them with the \nameref{def:Network_Station_Subsystem}, to provide core network functionality.
\end{definition}

A single base station and its subsystems can handle many different \nameref{def:Mobile_Station}s at once.

\paragraph{Network Station Subsystem}\label{par:Network_Station_Subsystem}
\begin{definition}[Network Station Subsystem]\label{def:Network_Station_Subsystem}
...
\begin{definition}[Operation Station Subsystem]\label{def:Operation_Station_Subsystem}
  The \emph{Operation Station Subsystem} (\emph{OSS}) (also sometimes called \emph{Network Management System} \emph{NMS}) is responsible for making sure the \nameref{def:Base_Station_Subsystem} and \nameref{def:Network_Station_Subsystem} are working together.
  It handles configuration of the radio interfaces in the \nameref{def:Base_Station_Subsystem} and the core network features of the \nameref{def:Network_Station_Subsystem}.
\end{definition}

\subsubsection{UMTS (3G)}\label{subsubsec:3G}
Really, the biggest difference in UMTS is that it used \nameref{def:CDMA} instead of \nameref{def:TDMA}/\nameref{def:FDMA}.
...
\begin{definition}[LTE]\label{def:LTE}
  \emph{LTE}'s or \emph{Long Term Evolution}'s main goals were to increase speed and capacity, while using a simpler IP-based network architecture.
  This means \textbf{ALL} data was now packet switched; there were no more circuit switched calls or domains any more.
  This was driven by the large increase in data usage compared to phone calls made.

  \textbf{The LTE radio interface was incompatible with previous 2G and 3G networks.}
...
\begin{definition}[Bearer]\label{def:Bearer}
  ...
  \item Non-GBR, e.g. FTP, web browsing
    ...
    \item No resources allocated, resources only given on demand from what is currently available (not currently consumed by GBR content).
...
\begin{figure}[h!tbp]
  \centering
  \includegraphics[scale=0.5]{./software-defined-networking.png}
