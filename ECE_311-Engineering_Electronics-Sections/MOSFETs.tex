...
\paragraph{Linear}\label{par:MOSFET_Linear_Region}
The linear region of operation for a \nameref{def:MOSFET} is actually a special case of the \nameref{subsubsec:MOSFET_Triode_Region} operation region.
A \nameref{def:MOSFET} will operate in the \emph{linear} region when the input voltage is quite low.
Namely, $\ACVoltage{\Drain\Source} \ll \OverdriveVoltage$, meaning $\ACVoltage{\Drain\Source} \ll \ThresholdVoltage$.

The linear region is technically a part of the \nameref{subsubsec:MOSFET_Triode_Region} region, but because $\DCVoltage{\Drain\Source}$ is so small, the squared term becomes negligible in \Cref{eq:MOSFET_Triode_Region-Current}, leaving us with \Cref{eq:MOSFET_Linear_Region-Current}.
...
Sometimes, the \nameref{subsubsec:MOSFET_Saturation_Region} is referred to as the location where channel \emph{pinchoff} occurs.
This is because the channel finishes its triangle and the drain's end of the triangle pinches off, see \Cref{fig:MOSFET-Pinchoff}.

\begin{figure}[h!tbp]
  \centering
  \includegraphics[scale=0.55]{./MOSFET-Pinchoff.png}
  \caption{MOSFET in Pinchoff (Saturation Region) \parencite[p.~259]{sedraTextbook7}}
  \label{fig:MOSFET-Pinchoff}
\end{figure}

When the \nameref{def:MOSFET} is in this operating region, it has a current relationship of \Cref{eq:MOSFET_Saturation_Region-Current}.

\begin{equation}\label{eq:MOSFET_Saturation_Region-Current}
  \ACCurrent{\Drain} = \frac{1}{2} k_{n} \OverdriveVoltage^{2}
\end{equation}

An equivalent circuit can be constructed for the operation of a \nameref{def:MOSFET} in the \nameref{subsubsec:MOSFET_Saturation_Region}, given in \Cref{fig:MOSFET-Large_Signal_Equivalent_Circuit}.

\begin{figure}[h!tbp]
  \centering
  \includegraphics[scale=0.70]{./MOSFET-Saturation-Large_Signal_Equivalent.png}
  \caption{Large-Signal \nameref*{def:MOSFET} Equivalent Circuit \parencite[p.~268]{sedraTextbook7}}
  \label{fig:MOSFET-Large_Signal_Equivalent_Circuit}
\end{figure}

\subsubsection{The Early Effect}\label{subsubsec:MOSFET-Early_Effect}
The Early effect, named after James M. Early, is an imperfection of \nameref{def:Transistor}s.
The effect can only occur in the \nameref{subsubsec:MOSFET_Saturation_Region} region, and allows some current to flow through the device even when the gate voltage is less than zero.

\begin{definition}[Channel Length Modulation Factor]\label{def:MOSFET-Channel_Length_Modulation_Factor}
  The \emph{channel length modulation factor}, or $\EarlyEffect$, is a constant that makes \nameref{def:Transistor}s behave in non-ideal ways.
\end{definition}
...
\subsection{Steps to Solve}\label{subsec:Steps_to_Solve}
\begin{enumerate}[noitemsep]
...
\item Find the gate-source voltage, $\ACVoltage{\Gate\Source}$, and determine if it is greater than $\ThresholdVoltage$.
  \begin{itemize}[noitemsep]
  \item If $\ACVoltage{\Gate\Source} \geq \ThresholdVoltage$, then the \nameref{def:MOSFET} is turned on, and is either in the \nameref{subsubsec:MOSFET_Saturation_Region} or \nameref{subsubsec:MOSFET_Triode_Region} region.
  \item If $\ACVoltage{\Gate\Source} < \ThresholdVoltage$, then the \nameref{def:MOSFET} is in the \nameref{subsubsec:MOSFET_Cutoff_Region}, and $\ACCurrent{\Drain} = 0$.
  \end{itemize}
\item Find the voltage at the drain, compared to the source ($\ACVoltage{\Drain\Source}$).
  \begin{itemize}[noitemsep]
  \item If $\ACVoltage{\Drain\Source} \geq \OverdriveVoltage$, then the \nameref{def:MOSFET} is in the \nameref{subsubsec:MOSFET_Saturation_Region}, and follows \Cref{eq:MOSFET_Saturation_Region-Current} for $\ACCurrent{\Drain}$.
  \item If $\ACVoltage{\Drain\Source} < \OverdriveVoltage$, then the \nameref{def:MOSFET} is in the \nameref{subsubsec:MOSFET_Triode_Region}, and follows \Cref{eq:MOSFET_Triode_Region-Current} for $\ACCurrent{\Drain}$.
  \end{itemize}
\end{enumerate}
