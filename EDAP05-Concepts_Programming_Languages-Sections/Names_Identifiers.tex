...
\begin{enumerate}[noitemsep]
\item At the second \texttt{print(x)}, \texttt{x} is \emph{in scope} (visible), but is \emph{dead}
\begin{minted}[frame=lines,linenos]{c}
int f(void) {
  int *x;
...
\item When executing inside of \texttt{g(y)}, the \texttt{x} in function \texttt{f} is \emph{alive}, but is \emph{out of scope} (not visible)
\begin{minted}[frame=lines,linenos]{python3}
def f(x):
  return g(7)
def g(y):
...
\begin{definition}[Static Scoping]\label{def:Variable_Static_Scoping}
  ...
  \begin{remark}[Lexical Scoping]\label{rmk:Variable_Lexical_Scoping}
    \nameref{def:Variable_Static_Scoping} is sometimes called \emph{lexical scoping}.
  \end{remark}
\end{definition}

\nameref{def:Variable_Static_Scoping} creates a tree-like structure, where each \nameref{def:Variable} declared in a program unit/block has a \nameref{def:Variable_Static_Parent}.
...
New \nameref{subsubsec:Variable_Static_Scope}s can be defined in the middle of executing code.
...
\begin{definition}[Block-Structured Language]\label{def:Block_Structured_Language}
  The use of \nameref{def:Block_Scope}s to create the \nameref{subsubsec:Variable_Static_Scope}s creates a \emph{block-structured language}.
\end{definition}
