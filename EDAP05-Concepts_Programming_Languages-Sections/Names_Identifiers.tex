...
\begin{definition}[Scope]\label{def:Variable_Scope}
  The \emph{scope} of a \nameref{def:Variable} is the range of statments in which the \nameref{def:Variable} is \nameref{def:Visible_Variable}.
\end{definition}

\begin{definition}[Visible]\label{def:Visible_Variable}
  A \nameref{def:Variable} is \emph{visible} in a statements if it can be referenced in that statment.
\end{definition}
