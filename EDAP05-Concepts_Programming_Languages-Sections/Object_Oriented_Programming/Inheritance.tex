...
\paragraph{Bounded Parametric Polymorphism Using Interfaces}\label{par:OOP_Bounded_Parametric_Polymorphism_Interfaces}
If we wanted to define a bounded interface, using interfaces, to implement the \texttt{PriorityQueue} from \Cref{subsec:Generic_ADTs}, we coudl write the scala code below.
...
The previous examples are \textit{too} restrictive, in that they require that the object \SemanticType{T} be able to compare itself against \emph{\textbf{any other}} object, when we just want to compare this type against inself, i.e.\ \SemanticType{Integer}s against \SemanticType{Integer}s.
