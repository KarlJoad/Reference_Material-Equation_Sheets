...
\item Use the determinant of the original \nameref{def:Quadratic_Form} matrix ($A$).
\end{enumerate}
...
\begin{blackbox}
  ...
  That means the \nameref{def:Quadratic_Form} is now:
  \begin{equation*}
    Ax_{1}^{2} + 2 Bx_{1}x_{2} + C x_{2}^{2} + 2 D x_{1} + 2 E x_{2} + F
  \end{equation*}
  Convert this to a \nameref{def:Real_Symmetric_Matrix}, $A$.
  \begin{equation*}
    A =
    \begin{pmatrix}
      A & B \\
  ...
  Similar to before, we convert the original \nameref{def:Quadratic_Form} to a \nameref{def:Real_Symmetric_Matrix} $B$.
  \begin{equation*}
    B = \begin{pmatrix}
      A & B & D \\
      B & C & E \\
