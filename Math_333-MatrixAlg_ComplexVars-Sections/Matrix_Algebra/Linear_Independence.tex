...
\begin{definition}[Linearly Independent]\label{def:Linearly_Independent}
  For a set of vectors to be \emph{linearly independent}, they are not \nameref{thm:Linearly_Dependent}.
  Namely, you will never find a scalar $c_{k} \neq 0$ such that \Cref{eq:Linearly_Dependent} is equal to 0.
...
\begin{example}[Lecture 16, Example 1]{Determine Linear Independence}
  Verify that $S$ is \nameref{def:Linearly_Independent}?
  \begin{equation*}
    S = \lbrace (1, 1, 0), (1, 0, 2), (0, 1, 0) \rbrace
  \end{equation*}
  \tcblower{}
  Now, we suppose that we can try to solve \Cref{eq:Linearly_Dependent}.
  \begin{align*}
  ...
  Just like in \Cref{ex:Determine Linear Dependence}, I will ignore the column vector of zeros.
  \begin{align*}
    \begin{pmatrix}
      1 & 1 & 0 \\
      1 & 0 & 1 \\
  ...
  Therefore, the set $S$ is \nameref{def:Linearly_Independent} because $c_{1}, c_{2}, c_{3} = 0$.
\end{example}
