...
\begin{example}[Lecture 8, Example 1]{Cauchy's Integral Theorem on a Non-Simple Path}
  ...
  \end{equation*}
  \tcblower{}
  The first step here is to ``break'' the union into two separate loops that we can then use \nameref{thm:Cauchys_Integral_Theorem} on.
  ...
  We were told that $C_{2}$ is oriented in the clockwise direction, but \nameref{thm:Cauchys_Integral_Theorem} states we must have the path be in the counterclockwise direction.
  ...
  Now, we need to determine what $f(z)$ from \Cref{thm:Cauchys_Integral_Theorem} is in both parts of the loops.
  ...
  Now that both integrals are on \nameref{def:Simple_Path} paths, are inside $\Omega = \ComplexNumbers$, and have $a$ inside them, we can use \Cref{thm:Cauchys_Integral_Theorem}.
  \begin{align*}
    \int_{C} \frac{2z + 3}{(z-1) (z+2)} dz &= \int_{C_{1}} \frac{\frac{2z + 3}{z+2}}{z-1} dz - \int_{C_{2}\text{, CCD}} \frac{\frac{2z + 3}{z-1}}{z+2} dz \\
...
And now, for completion's sake, we have an example using a sinusoid.
\begin{example}[Lecture 8, Example 2]{Using Cauchy's Integreal Theorem with a Sinusoid}
  ...
  \end{equation*}
  \tcblower{}
  We know that $z+3$ cannot be the denominator for \nameref{thm:Cauchys_Integral_Theorem}, because $a=-3$ is \textbf{not} inside the rectangle $R$ ($a=-3 \notin R$).
  \begin{equation*}
