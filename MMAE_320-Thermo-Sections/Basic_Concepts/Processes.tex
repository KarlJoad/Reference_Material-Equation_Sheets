...
Some \nameref{def:Process}es are special in that one \nameref{def:Property} remains constant.
There are several of these:
\begin{itemize}[noitemsep]
\item \nameref{def:Isothermal}
\item \nameref{def:Isobaric}
\item \nameref{def:Isometric}
\end{itemize}

\begin{definition}[Isothermal]\label{def:Isothermal}
  \emph{Isothermal} \nameref{def:Process}es are ones where the temperature $\Temp$ remains constant.
\end{definition}

\begin{definition}[Isobaric]\label{def:Isobaric}
  \emph{Isobaric} \nameref{def:Process}es are ones where the pressure $\Pressure$ remains constant.
\end{definition}

\begin{definition}[Isometric]\label{def:Isometric}
  \emph{Isometric} (\emph{Isochoric}) \nameref{def:Process}es are ones where the volume $\Volume$ remains constant.
\end{definition}
