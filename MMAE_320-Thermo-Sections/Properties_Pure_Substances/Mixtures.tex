...
\begin{example}{Change in Mixture Properties}
  ...
  Because we also know $\Vapor{\SpecificVolume} = \SI{1.6720}{\meter\cubed\per\kilo\gram}$, we can easily just solve for \Cref{eq:Single_Phase_Total_Volume} as the total volume.
  \begin{align*}
...
\begin{example}[Problem 4.23]{What's My State}
  ...
  \item Phase 2:
    ...
    \item They told us it was a \nameref{def:Saturated_Vapor}, so we know the value is somewhere on the saturated vapor line, meaning the volume is the same as $\Vapor{\SpecificVolume}$.
    \end{enumerate}
  \item Phase 3:
    \begin{enumerate}[noitemsep]
...
\begin{example}[Problem 4.112]{Changing State of R-134a}
  ...
  Now, we notice that $\SpecificVolume$ is somewhere between $\Fluid{\SpecificVolume}$ and $\Vapor{\SpecificVolume}$, meaning the R-134a is in its \nameref{def:Saturated_Mixture} phase right now.
