...
It works for car engines, steam-based electricity generating power plants, \nameref{def:Refrigerator}s, \nameref{def:Heat_Pump}s, etc.
...
\begin{definition}[Heat Pump]\label{def:Heat_Pump}
  \emph{Heat pump}s move \nameref{def:Heat} from a colder \nameref{rmk:Thermal_Reservoir} to a warmer one.
  A \emph{heat pump} is just another term for a \nameref{def:Refrigerator}.
\end{definition}

Because \nameref{def:Heat_Pump}s function identically to \nameref{def:Refrigerator}s and air conditioners, these equations are common between them.
...
\input{./MMAE_320-Thermo-Sections/2nd_Law_Thermo/Reversible_Irreversible_Processes}

\input{./MMAE_320-Thermo-Sections/2nd_Law_Thermo/Carnot_Cycle}

\input{./MMAE_320-Thermo-Sections/2nd_Law_Thermo/Clausius_Inequality}
