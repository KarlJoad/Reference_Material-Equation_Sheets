...
Due to these properties, the general energy balance equation for \nameref{def:Mixing_Chamber}s is shown in \Cref{eq:Mixing_Chamber}.
...
\begin{example}[Textbook Problem 6.78]{Mixing Chamber}
  A hot water stream of $\Temp_{In, 1} = \SI{80}{\degreeCelsius}$ enters a \nameref{def:Mixing_Chamber} with a mass flowrate of $\FlowRate{\Mass}_{In, 1} = \SI{0.5}{\kilo\gram\per\second}$.
  ...
  \tcblower{}
  \textbf{Concepts:} \\
  This is a \nameref{def:Mixing_Chamber} with steady flow.

  \textbf{Explore:} \\
  Our initial concepts about a \nameref{def:Mixing_Chamber} yield the equations:
  \begin{align*}
  ...
  The thing that affects the \nameref{def:Specific_Enthalpy} more is the temperature of the water, so we use Table A.4 rather than A.5.\@ \\
  There may need to be some extrapolation to get the \nameref{def:Specific_Enthalpy}.

  \textbf{Plan:} \\
  Use the tables in the textbook to find the \nameref{def:Specific_Enthalpy} for the various streams. \\
  ...
  \textbf{Generalize:} \\
  For a mass flow for a liquid under constant pressure, the \nameref{def:Specific_Enthalpy} ($\SpecificEnthalpy$) dominates the outcome.
\end{example}

\subsubsection{Heat Exchangers}\label{subsubsec:Heat_Exchangers}
\begin{definition}[Heat Exchanger]\label{def:Heat_Exchanger}
...
\begin{example}[Textbook Problem 6.82]{Heat Exchanger}
  R-134a at $\Pressure_{1} = \SI{700}{\kilo\pascal}$ and $\Temp_{1, In} = \SI{70}{\degreeCelsius}$ and moving at $\FlowRate{\Mass}_{1} = \SI{8}{\kilo\gram\per\minute}$ is cooled by water in a condenser until it exists as a \nameref{def:Saturated_Liquid} at the same pressure.
  ...
  \tcblower{}
  \textbf{Concepts:} \\
  \nameref{def:Heat_Exchanger} that is under steady state conditions.
  ...
  Using the tables for R-134a, we can find the \nameref{def:Specific_Enthalpy} at the inlet and outlet (1 and 2, respectively).
  ...
  Now, we apply \Cref{eq:Heat_Exchanger_Energy_Balance}:
  \begin{align*}
    \FlowRate{\Energy}_{In} &= \FlowRate{\Energy}_{Out} \\
  ...
  Next up, the various \nameref{def:Specific_Enthalpy} values from the tables.
  From Table A.12 at $\Pressure_{1} = \SI{700}{\kilo\pascal}$ we find that $\Temp_{1, Sat} = \SI{27}{\degreeCelsius}$.
  This means that the R-134a is a \nameref{def:Superheated_Vapor} right now.
  We move to Table A.13.
  \begin{equation*}
    \SpecificEnthalpy_{1, In} = \SI{308}{\kilo\joule\per\kilo\gram}
  \end{equation*}

  For the R-134a outlet's \nameref{def:Specific_Enthalpy}, we are told the refrigerant is a \nameref{def:Saturated_Liquid}, so $\SpecificEnthalpy_{1, Out} = \Fluid{\SpecificEnthalpy}$.
...
\subsubsection{Pipe/Duct Flow}\label{subsubsec:Pipe_Duct_Flow}
This is a subset of problems from the \nameref{def:Heat_Exchanger}.
\begin{table}[h!tbp]
  \centering
  \begin{tabular}{c}
    \toprule
    \nameref{subsubsec:Pipe_Duct_Flow} \\
    \midrule
    $\Change{\FlowRate{\Heat}} = \FlowRate{\Mass} \Change{\SpecificEnthalpy} \neq 0$ \\
    $\Change{\FlowRate{\Work}} = 0$ \\
    $\Change{\KineticEnergy} = 0$ \\
    ...
    \bottomrule
  \end{tabular}
  \caption{Pipe/Duct Flow Properteis}
  \label{tab:Pipe_Duct_Flow_Properties}
\end{table}

Something to note is that in \Cref{tab:Pipe_Duct_Flow_Properties}, we say $\Change{\FlowRate{\Work}} = 0$.
This only holds true for the purest case of this.
If a propeller were inserted to the piping, then the work done \textit{would} change.

Likewise, if the gas that is moving through the pipe is an ideal gas, we can use an ideal gas equation to determine $\Change{\SpecificEnthalpy} = \SpecificHeatPressure \Change{\Temp}$.

\begin{example}[Textbook Problem 6.98]{Pipe Flow}
  \nameref{def:Saturated_Liquid} water is heated in a steady-flow steam boiler at a constant pressure of $\Pressure = \SI{5}{\mega\pascal}$ at a rate of $\FlowRate{\Mass} = \SI{10}{\kilo\gram\per\second}$ to an outlet temperature of $\Temp_{Out} = \SI{350}{\degreeCelsius}$.
  Determine the rate of \nameref{def:Heat_Transfer} in the boiler?
  \tcblower{}
  \textbf{Concepts:} \\
  Same set of concepts of a \nameref{def:Heat_Exchanger}.
  Namely, this is a sub-class of problems.

  \textbf{Explore:} \\
  The only change in heat comes from the surrounding boiler, with $\FlowRate{\Heat}_{In}$.
  \begin{equation*}
    \FlowRate{\Heat}_{In} = \FlowRate{\Mass} \Change{\SpecificEnthalpy}
  \end{equation*}

  To find the \nameref{def:Specific_Enthalpy}, we can use Table A.4 or A.7 (if \nameref{def:Superheated_Vapor}).

  \textbf{Plan:} \\
  Use Table A.4 and/or Table A.7 from the back of the textbook to find the \nameref{def:Specific_Enthalpy}, $\SpecificEnthalpy$.
  Solve the heat energy-balance equation.

  \textbf{Solve:} \\
  From Table A.5 at $\Pressure = \SI{5}{\mega\pascal}$, we get:
  \begin{align*}
    \Temp_{Sat} &= \SI{263}{\degreeCelsius} \\
    \Fluid{SpecificEnthalpy} &= \SI{1154}{\kilo\joule\per\kilo\gram}
  \end{align*}

  From the previous table, we notice that the final temperature, is greater than the saturation temperature, $\Temp_{Out} > \Temp_{Sat}$.
  This means the fluid becomes a \nameref{def:Superheated_Vapor}.
  \begin{equation*}
    \SpecificEnthalpy_{Out} = \SI{3069}{\kilo\joule\per\kilo\gram}
  \end{equation*}

  Now, solving our energy balance equation:
  \begin{align*}
    \FlowRate{\Heat}_{In} &= \FlowRate{\Mass} \Change{\SpecificEnthalpy} \\
                          &= \FlowRate{\Mass} (\SpecificEnthalpy_{Out} - \SpecificEnthalpy_{In}) \\
                          &= 10 (3069 - 1154) \\
    &= \SI{19150}{\kilo\joule\per\kilo\gram}
  \end{align*}
\end{example}
